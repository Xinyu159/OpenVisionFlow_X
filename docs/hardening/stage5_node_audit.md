...
```
两趟 --full 的区别：2 / 502 个算子
  KMeansSegment   run1=D/TIMEOUT@S5  run2=A
  RandomCrop      run1=A             run2=C
```

**500/502 完全一致。** 只有 2 个算子在档位边界上抖 —— 两个都是 TIMEOUT 附近的
墙钟竞态（跑 2000 ms 预算的边缘）。所以：

- 结论可以当裁判用；
- 但这 2 个算子的档位**不要当成确定事实**，要复核就先给它们单独加预算
  （`--only KMeansSegment --budget-ms 20000`）。

另：总档位数字在几趟之间会在 ±2 内浮动（313/112/66 ↔ 310/113/68），
来源就是这类边界项，不是工具不稳定。
